            ...
            <table class="table table-striped align-middle table-row-dashed fs-6 gy-5 dt-table" id="products_table">
                <thead>
                    <tr class="text-start text-gray-600 fw-bold fs-7 text-uppercase gs-0">
                        ...
                        <th class="">Invoice</th>
                        <th>For</th>
                        <th class="text-end px-2">Placed on</th>
                    </tr>
                </thead>
                <tbody class="fw-semibold text-gray-600">
                    ...
                    <tr>
                        ...
                        <td class="text-gray-700 py-2" data-filter="{{ $due->grand_total }}">
                            <span class="fw-semibold">{{ number_format($due->grand_total) }}</span>
                        </td>
                        <td class="text-gray-700 py-2" data-filter="{{ $due->discount }}">
                            <span class="fw-semibold">{{ number_format($due->discount) }}</span>
                        </td>
                        <td class="text-gray-700 py-2" data-filter="{{ $due->paid }}">
                            <span class="text-info fw-semibold">{{ number_format($due->paid) }}</span>
                        </td>
                        ...
                        <td class="text-center py-2" data-filter="{{ $due->status }}">
                            ...
                            @elseif($due->status == 'pending approval')
                            <span class="badge badge-warning cursor-pointer" title="{{ ucfirst($due->status) }}">{{ ucwords($due->status) }}</span>
                            @endif
                        </td>
                        ...
                        <td class="text-gray-700 py-2" data-filter="<span>{{ Carbon\Carbon::parse($due->starting_date)->format('d/m/Y') }}</span>" data-order="{{ Carbon\Carbon::parse($due->starting_date)->format('Y-m-d') }}">
                            {{ Carbon\Carbon::parse($due->starting_date)->format('d M, Y') }}
                        </td>
        ...
        var KTAppEcommerceProducts = function () {
            ...
            var handleTypeFilter = () => {
                ...
                $(filterStatus).on('change', e => {
                    ...
                    datatable.column(6).search(value ? '^' + value + '$' : '', true, false).draw();
                });
            }
